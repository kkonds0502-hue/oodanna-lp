...
クライアント指示により、**kuramaproject.com のミニマル・タイポグラフィ主導**の思想に寄せています。
要件書6.2〜6.3のトークン定義から、次の点が意図的に変わっています。

| 項目 | 要件書 v1.0 | 現在 |
|---|---|---|
| 書体 | Shippori Mincho ＋ Cormorant Garamond ＋ Noto Sans JP | **Noto Sans JP 一本**（400/500/700/900） |
| アクセント色 | 朱赤 ＋ 金茶の2色 | **朱赤1色のみ**（価格と営業ステータス限定） |
| 面・装飾 | カード面・角丸・影・グラデーション | **すべて排除**（罫線と余白だけで区切る） |
| セクション背景 | 交互に色を変える | **フラット**（区切りは余白で表現） |
| 見出しサイズ | 最大88px | **最大172px**（FV）／148px（CONCEPT） |
| セクション余白 | 最大128px | **最大200〜280px** |
| 情報の見せ方 | カードグリッド | **縦一列のリスト＋「＋」で開く詳細** |

配色は要件書6.1の「黒基調」を維持しています（提供写真が暗背景で映えるため）。
